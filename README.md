...
Tokens are a temporary way of authenticating a user and allowing them into protected parts of an application. The token is an encoded data string which is used to verify the identity of a user and enable access.

- [ ] What steps can you take in your web apps to keep your data secure?

Use encoded data strings provided by the server of use details to allow access; These should only be temporary to avoid the use of local storage. Data should be kept in a server which is accessed through authenticated requests from client-side interaction.

- [ ] Describe how web servers work.

Your application will be run on a users device. Say a user wishes to login to an app. The users will input their data, client-side will send an HTTP request to the webserver. The web server then maps the data/files within the server-side and provides the client-side with a HTTP Response.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

CRUD is directly linked the HTTP methods: Get, post, put and Delete.
